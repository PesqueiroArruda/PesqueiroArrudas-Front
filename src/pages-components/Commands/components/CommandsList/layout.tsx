                ...
                <Flex align="center" gap={2}>
                  ...
                  {orderBy.toLowerCase() === prop.toLowerCase() && (
                    <motion.div
                      ...
                      style={{
                        transform: orderByDir === "asc" ? "rotate(0deg)" : "rotate(180deg)",
                        cursor: "pointer",
                      }}
                    >
        ...
        <Tbody>
          {items.length > 0 ? (
            items.map(({ _id, table, waiter, total, fishingType, isActive, discount, hasPendingOrders }) => (
              <Tr key={`list-command-${_id}`} h={20} cursor="pointer" _hover={{ bg: "blue.50" }}>
                <Td onClick={() => handleGoToCommandPage({ commandId: _id })}>{table}</Td>
                <Td onClick={() => handleGoToCommandPage({ commandId: _id })}>{waiter}</Td>
                <Td onClick={() => handleGoToCommandPage({ commandId: _id })}>
                  {parseToBRL(total - discount || 0)}
                </Td>
                <Td>{hasPendingOrders ? <LuChefHat size={24} color="red" /> : <LuChefHat size={24} color="transparent" />}</Td>
                <Td isNumeric>
                  {isActive === false && (
                    <Flex align="center" bg="green.300" display="inline-flex" gap={2} rounded={4} py={1} px={[1, 3]} mr={[1, 2]}>
                      <Icon as={MdVerified} fontSize={[18, 22, 24]} m={0} color="green.50" />
                      <Text fontSize={[14, 16, 18]} fontWeight="600" color="green.50">
                        Paga
                      </Text>
                    </Flex>
                  )}
                  {isAdmin && (
                    <Menu>
                      <MenuButton p={1} rounded={4} _hover={{ bg: "blue.50" }}>
                        <Icon as={CgOptions} fontSize={[16, 22]} color="blue.800" display="block" />
                      </MenuButton>
                      <MenuList>
                        <MenuItem
                          icon={<BiAddToQueue />}
